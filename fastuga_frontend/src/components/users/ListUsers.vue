                ...
                <v-layout align-center justify-center>
                    ...
                    <v-card>
                        ...
                        <v-data-table :headers="headers" :items="users" :search="search"
                            :sort-by="['name', 'email', 'type', 'active', 'nif']" :sort-desc="[true, true]"
                            class="elevation-1" :items-per-page=10 :footer-props="{
                                ...
                                <v-toolbar flat color="white">
                                    ...
                                    <v-dialog v-model="dialog" max-width="500px">
                                        <template v-slot:activator="{ on }">
                                            <v-btn color="primary" dark class="mb-2" v-on="on">New Employee Manager
                                            </v-btn>
                                        </template>
                                        <v-card ref="form">
                                            <v-card-title>
                                            ...
                                            <v-card-text>
                                                <v-container>
                                                    <v-row>
                                                        ...
                                                        <v-col cols="12" sm="6" md="4">
                                                            <v-text-field v-model="editedUser.nif" label="NIF">
                                                            </v-text-field>
                                                        </v-col>
                                                        <v-col class="d-flex" cols="12" sm="6">
                                                            <v-select v-model="editedUser.type" :items="userTypes"
                                                                label="User Type">
                                                            </v-select>
                                                        </v-col>
                                                        <v-col cols="12" sm="6" md="4">
                                                            <v-file-input v-model="selectedFile"
                                                                placeholder="Pick an avatar" prepend-icon="camera"
...
export default {
    data: () => ({
        ...
        userTypes: [{ text: 'Employee Manager', value: 'EM' }, { text: 'Employee Delivery', value: 'ED' }, { text: 'Employee Chef', value: 'Ec' }, { text: 'Customer', value: 'C' }]
    }),
    hasAlert: null,

    computed: {
        formTitle() {
            return this.editedIndex === -1 ? 'New Employee Manager' : 'edit'
        }
    },
