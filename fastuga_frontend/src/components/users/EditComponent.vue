                            ...
                            <v-card class="elevation-12">
                                ...
                                <v-form ref="form">
                                    <v-text-field label="Name" name="name" prepend-icon="person" type="text"
                                        v-model="user.name" :rules="[rules.required, rules.counter]"></v-text-field>

                                    <v-text-field id="oldPassword" label="Old Password" name="oldPassword"
                                        prepend-icon="lock" type="password" v-model="oldPassword"
                                        :rules="[rules.requiredPassword]"></v-text-field>

                                    <v-text-field id="New Password" label="New Password" name="newPassword"
                                        prepend-icon="lock" type="password" v-model="newPassword"
                                        :rules="[rules.password, rules.requiredPassword]"></v-text-field>
                                    ...
                                    <v-text-field id="passwordConfirmation" label="Password Confirmation"
                                        ...
                                        :rules="[rules.passwordConfirmation, rules.requiredPassword]"></v-text-field>
                                    <v-text-field v-if="user.nif" id="nif" label="NIF" name="nif" prepend-icon="lock"
                                        type="number" v-model="user.nif" :rules="[rules.nif]"></v-text-field>
                                    <v-file-input v-model="photo" placeholder="Pick an avatar" prepend-icon="camera"
                                        label="Avatar"></v-file-input>
                                    <v-card-actions>
                                        <v-spacer></v-spacer>
                                        <v-alert color="red" dense outlined type="error" v-if="hasAlert">{{ errorMsg }}
                                        </v-alert>
